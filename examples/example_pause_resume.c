/**
 * @file      example_pause_resume.c
 * @brief     定时器暂停&恢复示例
 * @author    huenrong (sgyhy1028@outlook.com)
 * @date      2026-03-28 13:13:22
 *
...
int main(int argc, char *argv[])
{
    ...
    int ret = gm_timer_init(test_timer, on_test_timer, GM_TIMER_REPEAT_FOREVER, 1000, &user_data);
    if (ret != 0)
    {
    ...
    uint64_t current_timestamp = 0;
    gm_time_get_current_timestamp_ms(&current_timestamp);
    printf("[%ld] timer is running, waiting for 3 seconds\n", current_timestamp);
    sleep(3);

    gm_time_get_current_timestamp_ms(&current_timestamp);
    printf("[%ld] pause timer\n", current_timestamp);
    gm_timer_pause(test_timer);

    printf("[%ld] timer is paused, waiting for 3 seconds\n", current_timestamp);
    sleep(3);

    gm_time_get_current_timestamp_ms(&current_timestamp);
    printf("[%ld] resume timer\n", current_timestamp);
    gm_timer_resume(test_timer);

    printf("[%ld] timer is resumed, waiting for 3 seconds\n", current_timestamp);
    sleep(3);
