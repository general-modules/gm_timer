/**
 * @file      example_stop_restart.c
 * @brief     定时器停止&重启示例
 * @author    huenrong (sgyhy1028@outlook.com)
 * @date      2026-03-28 13:23:47
 *
...
int main(int argc, char *argv[])
{
    ...
    int ret = gm_timer_init(test_timer, on_test_timer, 3, 1000, &user_data);
    if (ret != 0)
    {
    ...
    uint64_t current_timestamp = 0;
    gm_time_get_current_timestamp_ms(&current_timestamp);
    printf("[%ld] timer is running, waiting for stop\n", current_timestamp);
    while (!gm_timer_is_stopped(test_timer))
    {
        usleep(10 * 1000);
    }

    gm_time_get_current_timestamp_ms(&current_timestamp);
    printf("[%ld] timer is stopped, waiting for 5 seconds\n", current_timestamp);
    sleep(5);

    gm_time_get_current_timestamp_ms(&current_timestamp);
    printf("[%ld] restart timer\n", current_timestamp);
    gm_timer_restart(test_timer);
    sleep(5);
